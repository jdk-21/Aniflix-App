...
class ProfileSubBoxState extends State<ProfileSubBox> {
  ...
  ProfileSubBoxState(this.userid) {
    profilesubboxdata = ProfileRequests.getUserSubs(userid);
  }
  ...
  getLayout(BuildContext ctx) {
    return Container(
        ...
        child: RefreshIndicator(
            ...
            onRefresh: () async {
              ProfileRequests.getUserSubs(userid).then((newdata) {
                setState(() {
                  data = newdata;
                });
              });
  ...
  List<Widget> getWatchlistAsWidgets(BuildContext ctx, List<Show> watchlist) {
    List<Widget> watchlistWidget = [
      Container(
          padding: EdgeInsets.all(5),
          child: ThemeText("Subbox", fontSize: 30, fontWeight: FontWeight.bold))
    ];
