...
class AnimeScreenState extends State<AnimeScreen> {
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                if (!_sendAnalytics) {
                  ...
                  AppState.analytics.logViewItem(itemId: anime.id.toString(),
                      itemName: itemName,
                      itemCategory: "Show");
                }
                var episodeCount = 0;
                                  ...
                                  IconButton(
                                      onPressed: () {
                                        ...
                                        analytics.logEvent(
                                            ...
                                            parameters: {
                                              "show_id": anime.id,
                                              "watchlist_value": !_isInWatchlist
                                            });
                          ...
                          Row(
                            ...
                            children: <Widget>[
                              ...
                              OutlineButton(onPressed: () {
                                ...
                                child: Text("Staffel nicht Gesehen",
  ...
  setSeen(Anime anime) {
    if (_actualSeason != null && anime.seasons != null) {
      ...
      APIManager.setSeasonSeen(anime.seasons
          .elementAt(seasonNumber)
          .id).then((value) {
        setState(() {
  ...
  setUnseen(Anime anime) {
    if (_actualSeason != null && anime.seasons != null) {
      int seasonNumber = _actualSeason;
      APIManager.setSeasonUnSeen(anime.seasons
          .elementAt(seasonNumber)
          .id).then((value) {
        setState(() {
          anime.seasons[seasonNumber] = value;
...
List<Widget> getFriendsAsList(BuildContext ctx, FriendListData friendlistdata,
    Anime anime) {
    ...
    if (friend.status == 0) {
      if (friend.friend.id == CacheManager.userData.id) {
        friendlistwidget.add(IconListElement(friend.user.name,
          friend.user.avatar,
          ctx, onTap: () {
            APIManager.addRecommendNotification(
        ...
        friendlistwidget.add(IconListElement(
          friend.friend.name, friend.friend.avatar, ctx, onTap: () {
          APIManager.addRecommendNotification(
