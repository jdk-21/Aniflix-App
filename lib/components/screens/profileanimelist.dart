...
class ProfileAnimeListState extends State<ProfileAnimeList> {
  ...
  ProfileAnimeListState(this.userid) {
    profilelistdata = ProfileRequests.getUserSeen(userid);
  }
  ...
  getLayout(BuildContext ctx) {
    return Container(
        ...
        child: RefreshIndicator(
            ...
            onRefresh: () async {
              ProfileRequests.getUserSeen(userid).then((newdata) {
                setState(() {
                  data = newdata;
                });
              });
  ...
  List<Widget> getShowsAsWidgets(
      BuildContext ctx, List<UserSeenShow> watchlist) {
